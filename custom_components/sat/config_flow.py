...
class SatFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            valid = await self.async_test_gateway_connection()

            if not valid:
                self._errors["base"] = "auth"
            else:
                if sensor := self._data.get(CONF_INSIDE_SENSOR_ENTITY_ID):
                    self._data[CONF_SENSORS] = [sensor]
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(CONF_INSIDE_SENSOR_ENTITY_ID): selector.EntitySelector(
                    selector.EntitySelectorConfig(domain=SENSOR_DOMAIN, device_class=SensorDeviceClass.TEMPERATURE)
                ),
            }),
        )

    async def async_test_gateway_connection(self):
...
class SatOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_boiler(self, user_input=None) -> FlowResult:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Required(CONF_SENSOR_STALL, default=defaults.get(CONF_SENSOR_STALL)): selector.TimeSelector(),
            })
        )
    ...
    async def async_step_climates(self, user_input=None) -> FlowResult:
        if user_input is not None:
            ...
            if user_input.get(CONF_SENSORS) is None:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(CONF_SENSORS, default=defaults[CONF_SENSORS]): selector.EntitySelector(
                    selector.EntitySelectorConfig(domain=SENSOR_DOMAIN, device_class=SensorDeviceClass.TEMPERATURE, multiple=True)
                )
            })
        )
    ...
    def __getattr__(self, name):
        def function(user_input=None):
            return self.async_step_climates_destroy(name)
